...
const get10forcast = (data) => {
  ...
  return data.forecast.simpleforecast.forecastday.map(dayForecast => {
    let rObj = {};

    rObj.title = dayForecast.date.weekday_short;
    rObj.icon = weatherIcon(dayForecast.icon);
    rObj.high = dayForecast.high.fahrenheit;
    rObj.low = dayForecast.low.fahrenheit;
    return rObj;
  });
};
